...
public class AsyncForwardConnection extends AbstractAsyncClient{
	...
	public AsyncForwardConnection(AsyncForwardServer server, AsynchronousSocketChannel asyncSocket, int queueSize, int readBufSize, int writeBufSize) throws IOException{
		...
		this.partner = new AsyncForwardConnection(this.server, server, createAsyncSocket(this), queueSize, readBufSize, writeBufSize, this);
		setConnected(true);
	...
	AsyncForwardConnection(ServerDescription desc, AsyncForwardServer server, AsynchronousSocketChannel asyncSocket, int queueSize, int readBufSize, int writeBufSize, AsyncForwardConnection partner) throws IOException {
		super(asyncSocket, queueSize, readBufSize, writeBufSize);
		setConnected(true);
		this.forwardServer = server;
